console.log("Edit sales engineer loaded")
import {
    clean_email,
    clean_name,
    clean_username,
...
const sales_engineer_id = JSON.parse(document.getElementById("id").textContent);
...
const check_username = () =>{
    ...
    const username_url = `${base_url}/check-username`

    fetch(`${username_url}?username=${encodeURIComponent(username.value)}&id=${sales_engineer_id}`,{
        method: "GET",
        headers:{
...
const check_email = () =>{
    ...
    const email_url = `${base_url}/check-email`

    fetch(`${email_url}?email=${encodeURIComponent(email.value)}&id=${encodeURIComponent(sales_engineer_id)}`,{
        method: "GET",
        headers:{
            "Content-Type": "application/json"
        }
